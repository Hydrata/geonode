...
def get_visible_resources(
    ...
    return permissions_registry.get_visible_resources(
        ...
        unpublished_not_visible=unpublished_not_visible,
        private_groups_not_visibile=private_groups_not_visibile,
...
ResourceGroupsAndMembersSet = collections.namedtuple(
...
class AdvancedSecurityWorkflowManager:
    ...
    def assignable_perm_condition(perm, resource_type):
        _assignable_perm_policy_condition = (
            ...
            or (perm in DATASET_EDIT_DATA_PERMISSIONS and resource_type in DATA_EDITABLE_RESOURCES_SUBTYPES)
            or (perm not in (DOWNLOAD_PERMISSIONS + DATASET_EDIT_DATA_PERMISSIONS))
        )
        logger.debug(
            f" perm: {perm} - resource_type: {resource_type} --> assignable: {_assignable_perm_policy_condition}"
        )
    ...
    def compute_resource_groups_and_members_set(uuid: str, /, instance=None, group=None) -> ResourceGroupsAndMembersSet:
        ...
        return ResourceGroupsAndMembersSet(
            anonymous_group, registered_members_group, user_groups, resource_groups, group_managers
        )
    ...
    def get_workflow_permissions(
        ...
        def safe_remove(perms, perm):
            perms.remove(perm) if perm in perms else None

        if _resource:
            _resource = _resource.get_real_instance()

            AdminViewPermissionsSet = AdvancedSecurityWorkflowManager.compute_admin_and_view_permissions_set(
                uuid, instance=_resource
            )
            ResourceGroupsAndMembersSet = AdvancedSecurityWorkflowManager.compute_resource_groups_and_members_set(
                uuid, instance=_resource, group=_resource.group
            )
            ...
            if not AdvancedSecurityWorkflowManager.is_auto_publishing_workflow():
                ...
                if _resource.owner not in ResourceGroupsAndMembersSet.managers:
                    ...
                    if not AdvancedSecurityWorkflowManager.is_simple_publishing_workflow() and (
                        _resource.is_approved or _resource.is_published
                    ):
            ...
            if not AdvancedSecurityWorkflowManager.is_auto_publishing_workflow():
                ...
                if ResourceGroupsAndMembersSet.managers:
                    for user in ResourceGroupsAndMembersSet.managers:
                        ...
                        prev_perms += (
                            AdminViewPermissionsSet.view_perms.copy() + AdminViewPermissionsSet.admin_perms.copy()
                        )
                        prev_perms = list(set(prev_perms))
                ...
                if ResourceGroupsAndMembersSet.resource_groups:
                    ...
                    for group in groups:
                        if group not in (
                            ResourceGroupsAndMembersSet.anonymous_group,
                            ResourceGroupsAndMembersSet.registered_members_group,
                        ):
                            try:
            ...
            if ResourceGroupsAndMembersSet.anonymous_group:
                prev_perms = (
                    _perm_spec["groups"].get(ResourceGroupsAndMembersSet.anonymous_group, [])
                    if isinstance(_perm_spec["groups"], dict)
                    else []
                )
                ...
                if not AdvancedSecurityWorkflowManager.is_auto_publishing_workflow():
                    if (
                        (
                            AdvancedSecurityWorkflowManager.is_simple_publishing_workflow()
                            or AdvancedSecurityWorkflowManager.is_advanced_workflow()
                        )
                        and not _resource.is_published
                    ) or (
                        AdvancedSecurityWorkflowManager.is_simplified_workflow()
                        and not (_resource.is_approved or _resource.is_published)
                    ):
            ...
            if ResourceGroupsAndMembersSet.registered_members_group and getattr(
                ...
                prev_perms = (
                    _perm_spec["groups"].get(ResourceGroupsAndMembersSet.registered_members_group, [])
                    if isinstance(_perm_spec["groups"], dict)
                    else []
                )
                if approval_status_changed and (_resource.is_approved or _resource.is_published):
    ...
    def handle_moderated_uploads(uuid: str, /, instance=None) -> object:
        ...
        if _resource:
            if not AdvancedSecurityWorkflowManager.is_auto_publishing_workflow():
                ...
                ResourceBase.objects.filter(uuid=_resource.uuid).update(
                    is_approved=False, was_approved=False, is_published=False, was_published=False
                )
            ...
            queryset = chain(
                ...
                (
                    get_objects_for_group(
                        group.group, ["base.view_resourcebase", "base.change_resourcebase"], any_perm=True
